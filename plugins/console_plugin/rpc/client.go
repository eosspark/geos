...
func (c *Client) send(ctx context.Context, op *requestOp, msg interface{}) error {
	...
	case c.requestOp <- op:
		...
		//}})
		rpclog.Info("sending: %s", msg) //TODO
		err := c.write(ctx, msg)
		c.sendDone <- err
		return err
	case <-ctx.Done():
...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		rpclog.Info(fmt.Sprintf("reconnect failed: %v", err))
		return err
	}
	select {
...
func (c *Client) handleNotification(msg *jsonrpcMessage) {
	if !strings.HasSuffix(msg.Method, notificationMethodSuffix) {
		rpclog.Debug("dropping non-subscription message", "msg", msg)
		return
	}
	...
	if err := json.Unmarshal(msg.Params, &subResult); err != nil {
		rpclog.Debug("dropping invalid subscription message", "msg", msg)
		return
	}
